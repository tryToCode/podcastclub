<template>
  <nav class="fixed w-full z-30 top-0 text-white"
        :class="bgNav">

        <div class="w-full container mx-auto max-w-5xl mx-8 
            flex flex-wrap items-center justify-between mt-0 py-3">
                
            <div class="flex items-center pl-4 lg:pl-0">
                <nuxt-link class="tracking-tighter no-underline hover:no-underline 
                    font-bold text-2xl lg:text-4xl"
                    :class="bgText"
                    id="reloadPage"
                    to="/"
                    @click.native="reload">
                    Podcast Club
                </nuxt-link>
            </div>

            <div class="block lg:hidden pr-4">
                <button id="nav-toggle" class="flex items-center p-1 text-orange-800 hover:text-gray-900">
                    <svg class="fill-current h-6 w-6" viewBox="0 0 20 20" 
                    xmlns="http://www.w3.org/2000/svg">
                    <title>Menu</title>
                    <path d="M0 3h20v2H0V3zm0 6h20v2H0V9zm0 6h20v2H0v-2z"/></svg>
                </button>
            </div>

            <div class="w-full flex-grow lg:flex lg:items-center lg:w-auto hidden lg:block 
                mt-2 lg:mt-0 bg-white lg:bg-transparent text-black p-4 lg:p-0 z-20">
                <ul class="list-reset lg:flex justify-end flex-1 items-center">
                    <li class="mr-3">
                        <nuxt-link class="inline-block py-2 px-4 font-bold no-underline 
                            cursor-pointer" 
                        :class="bgText"
                        to="/">
                            Home
                        </nuxt-link>
                    </li>
                    <li class="mr-3">
                        <nuxt-link class="inline-block no-underline
                            hover:text-underline py-2 px-4 cursor-pointer" 
                            :class="bgText"
                        to="/show">
                        Show
                        </nuxt-link>
                    </li>
                    <li class="mr-3">
                        <button v-if="!loggedIn || !isAuthenticated"
                            @click="login"
                            class="mx-auto lg:mx-0 hover:underline
                                font-bold rounded mt-4 lg:mt-0 py-2 px-4"
                            :class="bgBtn">
                            Login
                        </button>
                        <div v-else>
                            <base-drop-down />
                        </div>
                    </li>
                </ul> <!--navbar right -->
            </div> <!--navbar right div closed -->
        </div>
        
        <hr class="border-b border-gray-100 opacity-25 my-0 py-0" />
    </nav>
</template>

<script>
import { mapState } from 'vuex'
import BaseDropDown from './Base/BaseDropDown.vue'

export default {
    name: 'navbar',

    data() {
        return {
            isOpen: false,
            windowTop: 0,
            isAuthenticated: false,
        }
    },

    components: {
        BaseDropDown
    },

    computed: {
        ...mapState({
            loggedIn: state => state.auth.isAuthenticated
        }),

        bgNav: function() {
            return this.windowTop > 10 
                ? 'bg-white shadow' 
                : 'gradient' 
        },

        bgText: function() {
            return this.windowTop > 10 
                ? 'text-black' 
                : 'text-white' 
        },

        bgBtn: function() {
            return this.windowTop > 10 
                ? 'gradient text-white' 
                : 'bg-white text-gray-800 opacity-75'
        }
    },

    mounted() {
        if (process.browser)
            window.addEventListener("scroll", this.onScroll)
            
        if (localStorage.getItem('apiUrl')) {
            let isAuthenticated = null
            JSON.parse(localStorage.getItem('apiUrl'), (key, value) => {
                if (key === 'isAuthenticated') {
                    isAuthenticated = value
                    return
                }
            })
            this.isAuthenticated = isAuthenticated
        }
    },

    beforeDestroy() {
        if (process.browser)
            window.removeEventListener("scroll", this.onScroll)
    },

    methods: {
        reload() {
            if (process.browser) {
                window.location.reload(true)
                localStorage.clear()
            }
        },

        login() {
            this.$emit('login')
        },

        onScroll(e) {
            if (process.browser)
                this.windowTop = window.top.scrollY
        }
    }
}
</script>